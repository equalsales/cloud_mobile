...
class greyorder_list extends StatefulWidget {
  var xcompanyid, xcompanyname, xfbeg, xfend, xbranch, xorderno;
  var Title = 'Branch List';
  greyorder_list(
      {Key? mykey, companyid, companyname, fbeg, fend, branch, orderno, caption = ''})
      : super(key: mykey) {
  ...
  @override
  greyorder_ListState createState() => greyorder_ListState();
}

class greyorder_ListState extends State<greyorder_list> {
  List _orderlist = [];
  List _orgorderlist = [];
  List _orderSelected = [];
  ...
  Future<bool> getorderlist() async {
    ...
    uri = '${globals.cdomain}/getgreyjoborderlist/?branch=$branch&dbname=$db&user=$user&aorddetid=0&ordid=';

    print(" greygetorderlist "+uri);
